...
function getLastReview(objArr) {
    return `${objArr[objArr.length -1].name} gave the restaurant a ${objArr[objArr.length -1].rating}, and their feedback was: ${objArr[objArr.length -1].feedback}`
  } 
...
*/
 ...
 function getReviewByRating(objArr, rated) {
    ...
    for(let i = 0; i < objArr.length; i++){
      if(objArr[i].rating >= rated && objArr[i].rating < rated + 1){
...
*/
  function getLongReviews(objArr) {
    const longReviews = [];
    for(let i = 0; i < objArr.length; i++){
      if(objArr[i].feedback.length > 15){
        longReviews.push(objArr[i]);
      }
    }
    return longReviews
  }
